...
import { householdMembers, households, members, newFamilyRegistrations } from "@/db/schema";
...
export async function GET(request: Request) {
  ...
  const [registration] = await db.select({ id: newFamilyRegistrations.id, cardType: newFamilyRegistrations.cardType, reviewStatus: newFamilyRegistrations.reviewStatus, reviewNote: newFamilyRegistrations.reviewNote, createdAt: newFamilyRegistrations.createdAt, reviewedAt: newFamilyRegistrations.reviewedAt }).from(newFamilyRegistrations).where(eq(newFamilyRegistrations.memberId, current.id)).orderBy(desc(newFamilyRegistrations.createdAt)).limit(1);
