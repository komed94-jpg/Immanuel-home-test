...
export default async function ImmanuelBasicStudyPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  const { page } = await searchParams;
  const startPageKey = page && immanuelBasicCourse.pages.some((item) => item.key === page) ? page : undefined;
  return <Layout>
    ...
    <StudyWorkbook key={startPageKey ?? "default"} course={immanuelBasicCourse} startPageKey={startPageKey} />
    <section className="study-section" aria-label="다음 성경공부">
      ...
      <div className="study-material-grid"><article className="study-material-card"><small>임마누엘의 길 02</small><h3>그럼에도 불구하고, 여전히 하나님은 나를 사랑하신다 Ⅱ</h3><p>부끄러움이 아니라 아버지의 시선으로 나를 보는 법</p><Link href="/bible-study/immanuel-basic-2#study-content" className="text-action">2권 웹 교재 열기</Link></article></div>
    </section>
  </Layout>;
}
