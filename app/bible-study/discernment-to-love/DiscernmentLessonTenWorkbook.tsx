...
import { lessonTenPages } from "./lesson-ten";
import type { Block, Question } from "./lesson-ten/types";
...
const course = {
  ...
  lessonSlug: "discernment-lesson-10",
  title: "10과 · AI를 어떻게 사용해야 하는가",
  pages: lessonTenPages
};
...
export function DiscernmentLessonTenWorkbook({ startPage }: { startPage?: string }) {
  ...
  async function load() {
    const response = await fetch("/api/member/study/discernment-lesson-ten", { cache: "no-store" });
  ...
  function selectPage(nextIndex: number) {
    ...
    url.searchParams.set("lesson", "10");
  ...
  async function saveAnswer(questionKey: string, answer: string) {
    ...
    const response = await fetch("/api/member/study/discernment-lesson-ten", {
      method: "POST",
  ...
  return <section className="web-study-shell">
    ...
    <article className="web-study-page" id="study-content" ref={contentRef} tabIndex={-1}>
      ...
      {needsLogin && <div className="web-study-login-callout"><strong>읽기는 누구나 할 수 있습니다.</strong><p>답변 저장, 공부 날짜 기록, 진도 관리는 로그인한 교인에게 열립니다.</p><a className="primary-link" href={`/login?returnTo=${encodeURIComponent(`/bible-study/${course.slug}?lesson=10&page=${page.key}#study-content`)}`}>로그인하여 이 페이지부터 기록하기</a></div>}
